...
The subsequent `0091_qualifier_other_relation` revision adds the Other relationship code and expands saved relationship labels to 100 characters without rewriting existing answers. Revision `0092_whatsapp_matching_fields` then stores each WhatsApp broadcast's safe spreadsheet-heading catalog and each link's selected OR-matching fields; existing links retain legacy matching until an operator saves a selection. Revision `0093_phone_welcome` adds the phone-scoped welcome prerequisite and traveller welcome outbox. Apply the full migration chain before serving the current code. Readiness defaults and Compose configuration now expect `0093_phone_welcome`; update any explicit `EXPECTED_DATABASE_SCHEMA_REVISION` override in the deployment environment and recreate backend/worker containers. Downgrading `0091` is refused while stored Other answers or labels longer than 80 characters would be lost.
