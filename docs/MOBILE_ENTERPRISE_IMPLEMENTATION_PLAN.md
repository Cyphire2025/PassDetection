...
- Protect sensitive routes from screenshots, screen recording, and app-switcher snapshots where the platforms permit it.
...
| F-17 | P0 | Sensitive screens allow capture/task snapshots | Platform capture prevention plus privacy overlay on inactive/background; graceful unsupported-platform behavior | Screenshot/recording/task-switcher tests on Android and iOS | development complete - manual gate: source contracts green; platform capture verification deferred |
...
| F-19 | P2 | Keys remain accessible after first unlock | Split key policy by data sensitivity; use strongest compatible accessibility/user-presence policy and short leases for high-risk material | Locked-device/background/reboot/biometric-change tests on both platforms | development complete - manual gate: key-policy tests green; locked-device/biometric proof deferred |
...
- Screenshot, recording, and task-switcher privacy.
