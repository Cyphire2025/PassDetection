...
test("all three passport images expose crop and effective open-new-tab actions", () => {
...
test("editor always fetches the immutable original through authenticated blob API", () => {
  assert.match(editor, /passportsApi\.getOriginalImage/);
  assert.match(api, /getOriginalImage:[\s\S]*?responseType: "blob"/);
  assert.match(endpoints, /images\/\$\{imageType\}\/original/);
...
test("editor supports pointer, touch, keyboard, rotation, dimming, reset, and focus containment", () => {
  ...
  assert.match(editor, /Reset crop/);
