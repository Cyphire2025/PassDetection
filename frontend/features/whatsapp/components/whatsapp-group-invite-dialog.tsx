...
export function GroupInvitePreviewDialog({ group, targetRecipient, bulkRecipients, hiddenSelectedCount = 0, isSending, onClose, onSend }: MessagePreviewDialogProps) {
  ...
  const draftKey = JSON.stringify({ messageContent, groupInviteLink, selectionMode, selectedIds, bulkIds });
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      if (bulkIds) {
        requestBulkPreview({ groupId: group.id, messageType: "group_invite", recipientIds: bulkIds, previewRecipientId: effectivePreviewRecipientId, overrides: { messageContent, groupInviteLink }, signal: controller.signal }, {
          onSuccess: (response) => {
            if (controller.signal.aborted || requestSequence.current !== sequence) return;
        ...
        requestPreview({ groupId: group.id, draft: {
          message_type: "group_invite", message_content: messageContent, group_invite_link: groupInviteLink,
  ...
  const canSend = Boolean(previewCurrent && !archived && detail?.recipient_opt_in_confirmed && resolvedContent && validLink && eligibleCount > 0 && targetAllowed && !welcomeReason
  ...
  const handleSend = async (event: FormEvent) => {
    ...
    await submitPayload({ passportIntro: "", passportLink: "", messageContent: resolvedContent, groupInviteLink: resolvedLink,
      headerImage: null, headerImageId: null, supportContactIds: null,
      recipientIds: bulkIds ?? (selectionMode === "custom" ? selectedIds : null),
      ...(bulkMode ? { bulkDraft: { messageContent: messageContent === null ? null : resolvedContent, groupInviteLink: groupInviteLink === null ? null : resolvedLink } } : {}),
    });
  };
  ...
  return <DialogFrame title={`${bulkMode ? "Resend" : targetRecipient ? targetRecipient.action === "retry" ? "Retry" : "Resend" : "Send"} Group Invite`} onClose={onClose} isBusy={pending} widthClass="max-w-6xl" layout="composer" eyebrow="WhatsApp communications" description={group.name}>
    <form className="flex min-h-0 flex-1 flex-col" onSubmit={handleSend}>
          ...
          <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)]">
            <div className="min-w-0 space-y-5">
              <MessageComposerSection title="Group invitation" description="Edit the invitation and group link. The greeting and sign-off stay exactly as shown in the preview.">
                ...
                {bulkMode && <p className="rounded-lg bg-blue-50 p-3 text-xs leading-5 text-blue-800">Unchanged fields keep each recipient’s saved invitation and group link. Edits apply to every selected recipient.</p>}
            ...
            <aside className="min-w-0 space-y-3 lg:sticky lg:top-0">
              <MessageDeliveryPreview preview={preview} previewIsCurrent={previewCurrent} previewFailed={Boolean(error)} messageType="group_invite" headerImagePreview={null} headerImageId={null}>
      ...
      <div data-testid="whatsapp-composer-footer" className="flex shrink-0 flex-col gap-3 border-t border-slate-200 bg-white px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-7">
        <p role="status" className="text-xs text-slate-500">{pending ? "Submitting your group invites..." : canSend ? "Message checked and ready to send." : "Complete the invitation and official group link, then review the preview."}</p>
