...
export function RecipientSelectionPanel({
  ...
  onSelectAll: () => void;
  onClear: () => void;
  onReview: (messageType: "welcome" | "passport_link" | "group_invite") => void;
  disabled: boolean;
  error: string | null;
  notice: string | null;
}) {
  return (
    <aside aria-label="Selected recipients" className={`${selectedCount ? "hidden xl:block" : ""} rounded-2xl border border-slate-200 bg-white p-5 xl:sticky xl:top-0`}>
      ...
      <p className="mt-1.5 text-sm leading-relaxed text-slate-500">
        {selectedCount ? "Choose the saved message you want to send again to these recipients." : "Select people from the list to resend a saved welcome, passport link, or group invite in one action."}
      </p>
      ...
      <div className="mt-5 grid gap-2.5 border-t border-slate-100 pt-5 sm:grid-cols-2 xl:grid-cols-1">
        ...
        <Button type="button" variant="secondary" disabled={!selectedCount || disabled} onClick={() => onReview("group_invite")} className="justify-start gap-2.5 py-3"><Users className="h-4 w-4 shrink-0" /> Resend group invite</Button>
      </div>
  ...
  return (
    <section aria-label="Selected recipient actions" className="shrink-0 border-t border-blue-100 bg-white px-4 py-3 shadow-[0_-4px_16px_rgba(15,23,42,0.04)] xl:hidden">
      ...
      <div className="mt-2.5 grid grid-cols-2 gap-2">
        ...
        <Button type="button" variant="secondary" disabled={disabled} onClick={() => onReview("group_invite")} className="gap-1.5 px-2 text-xs"><Users className="h-3.5 w-3.5" />Resend group invite</Button>
      </div>
