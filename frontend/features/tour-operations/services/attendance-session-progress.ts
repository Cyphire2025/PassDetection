...
export function readAttendanceSessionProgress(
  groupId: string,
  sessionId: string,
): AttendanceSessionProgress | null {
  return readProgressMap(groupId)[sessionId] ?? null;
}
...
export function writeAttendanceSessionProgress(
  ...
  const map = readProgressMap(groupId);
  map[sessionId] = {
  ...
  writeProgressMap(groupId, map);
}

export function mergeAttendanceSessionProgress(
  groupId: string,
...
export function reconcileAttendanceSessionProgress(
  groupId: string,
  sessions: AttendanceSession[],
) {
  const map = readProgressMap(groupId);
  ...
  if (changed) writeProgressMap(groupId, map);
}

function readProgressMap(groupId: string): Record<string, AttendanceSessionProgress> {
  if (typeof window === "undefined") return {};
  const storageKey = getCurrentUserStorageKey(groupId);
...
function writeProgressMap(
  groupId: string,
  map: Record<string, AttendanceSessionProgress>,
) {
  const storageKey = getCurrentUserStorageKey(groupId);
  if (!storageKey) return;
  try {
...
function getCurrentUserStorageKey(groupId: string) {
  const userId = useAuthStore.getState().user?.id;
  return userId
    ? `${STORAGE_KEY_PREFIX}:user:${userId}:group:${groupId}`
    : null;
}
