...
const groupCard = read("./components/group-control-card.tsx");
...
test("group discovery is bounded and GC access mutations are revision safe", () => {
  ...
  assert.match(api, /updateGroupControl:[\s\S]{0,250}apiClient\.put<RawGroupAccess>\(\s*`\$\{ROOT\}\/groups\/\$\{control\.id\}`/);
  const removal = api.slice(api.indexOf("  removeGroup:"), api.indexOf("  updateGroupControl:"));
  assert.match(removal, /apiClient\.delete\(\s*`\$\{ROOT\}\/groups\/\$\{control\.id\}\/app-setup`/);
  assert.match(removal, /agencyParams\(agencyId, \{ expected_revision: control\.revision \}\)/);
  assert.doesNotMatch(removal, /apiClient\.put/);
  assert.match(api, /revokeGroupAccess:[\s\S]{0,250}apiClient\.delete\(`\$\{ROOT\}\/groups\/\$\{groupId\}`/);
...
test("new groups explicitly enable every mobile role while settings live inside the trip", () => {
  ...
  assert.match(controls, /<GroupControlCard/);
  assert.match(groupCard, /Open trip/);
  assert.match(groupCard, /ROUTES\.dashboard\.gcAppGroup\(group\.id\)/);
...
test("publishing remains inside App Controls with fixed itinerary and categorized common documents", () => {
  ...
  assert.match(workspace, /useGcAppAnnouncements\(agencyId, groupId, announcementPage, 25, tab === "announcements" && !control\.data\?\.gc_removed_at\)/);
  assert.match(workspace, /useGcAppGroupContent\(agencyId, groupId, tab === "documents" && !control\.data\?\.gc_removed_at\)/);
