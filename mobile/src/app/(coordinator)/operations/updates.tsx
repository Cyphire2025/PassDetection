...
export default function CoordinatorUpdatesScreen() {
  ...
  const renderItem = useCallback(({ item }: SectionListRenderItemInfo<Row, UpdatesSection>) => {
    ...
    const Icon = notification.priority === 'emergency'
      ? CircleAlert
        ...
        <GlassCard style={[styles.card, notification.priority === 'emergency' && styles.emergency, !notification.read_at && styles.unread]}>
          <View style={styles.heading}>
            <View style={[styles.icon, notification.priority !== 'normal' && styles.importantIcon]}>
              <Icon color={notification.priority === 'normal' ? colors.greenDeep : colors.danger} size={20} />
            </View>
      ...
      <SectionList<Row, UpdatesSection>
        ...
        ListHeaderComponent={
          <View style={styles.header}>
            <OperationHeader title="Updates" subtitle={trips.selectedTrip?.name || 'Selected group'} />
            ...
            {notifications.isError && announcements.isError ? (
              <ContentError message="Updates are not available on this device." onRetry={() => { void notifications.refetch(); void announcements.refetch(); }} />
            ) : null}
          </View>
        }
        ListEmptyComponent={
          !notifications.isPending && !announcements.isPending
            ? <ContentEmpty title="You are up to date" message="Published changes and coordinator alerts will appear here." />
            : null
        }
        ListFooterComponent={
...
const styles = StyleSheet.create({
  ...
  icon: { width: 38, height: 38, borderRadius: 13, backgroundColor: colors.greenSoft, alignItems: 'center', justifyContent: 'center' },
  ...
  unreadDot: { width: 9, height: 9, borderRadius: 5, backgroundColor: colors.green },
  pressed: { opacity: 0.7 },
});
