...
- Direct Android FCM delivery through the VPS using the Expo Notifications native library, private notification visibility, durable read state, strict deep-link routing, and push-triggered refresh.
...
Every production build must provide the API URL, `EXPO_PUBLIC_APP_ENV=production`, and `EXPO_PUBLIC_DEMO_MODE=false`. Android push uses a native FCM token and does not require an Expo account or EAS project ID. To enable optional EAS Update, provide the EAS project ID, Expo owner, canonical Update URL containing the same project UUID, and a build-time verification certificate. Unsigned remote updates are rejected by configuration. The private update-signing key is used only by the protected publishing job and must never enter the repository or application build.

### Android phone notifications

The delivery path is VPS → Google FCM → Android. The app calls `getDevicePushTokenAsync()` and registers `provider=fcm` with its authenticated installation. `GOOGLE_SERVICES_JSON` must identify the approved Firebase project and the package `com.globalconnects.groupcompanion`; the backend's FCM sender must use the same Firebase project. The Firebase service-account private key belongs only on the VPS and must never enter the mobile bundle. Google accepting a message does not confirm that the phone displayed it.

Build and install a new signed production APK containing this change. Existing Expo tokens cannot be converted to FCM tokens on the VPS. On the first online registration after upgrading, the existing login and installation are reused and the legacy Expo registration marker is replaced by the native FCM marker. The Metro adapter suppresses Expo's import-time relay auto-registration and clears its legacy preference when native registration runs; it does not clear trip data or authentication. Token refresh, foreground retries, permission checks and sign-out de-registration continue to use the authenticated backend.

Phone push uses native FCM on Android and native APNs on iOS, with the VPS sending directly to Google or Apple. iOS registration includes `apns_environment`, read from the signed provisioning profile through `expo-application`; it is independent of the app's API environment. App Store distributions without an embedded profile use production only after native APNs token acquisition succeeds. Unknown signing environments fail with an actionable build-configuration status. Native token-change events reuse the supplied token to avoid recursively requesting another token on iOS. Expo relay uploads remain disabled on both platforms.

Authored phone alerts open the authenticated **Phone alerts** inbox, accessible from the notification settings card even without a selected trip. The inbox reads only the current account's `gc_alert` recipients, supports pagination and mark-as-read, and refreshes on foreground delivery or app resume. It requires a connection to load or mark alerts read; the current session's already-loaded list stays in memory while offline. Existing trip Updates retain their separate offline cache. Announcement publication itself is an in-app content action; sending an authored phone alert is a separate dashboard action.

Actual iPhone push requires a push-enabled App ID, an Apple Developer Program signing team/provisioning profile, and protected APNs credentials configured on the VPS. A free Personal Team can continue testing other app features: explicitly set `GC_IOS_PUSH_NOTIFICATIONS_ENABLED=false` for a private development/preview native generation. This removes only the push entitlement and makes the app explain why phone delivery is unavailable. It does not change existing native files until ordinary native generation is run, does not remove other capabilities, and is rejected for strict production builds. Leave the flag unset for a push-enabled build. No credential belongs in this flag, the app bundle, or `EXPO_PUBLIC_*`.

After installing a new signed build, validate foreground, background and closed-app taps, permission denial/re-enable, account changes, token renewal, and both APNs environments on physical phones as applicable. Unit tests and unsigned native CI compilation do not prove actual phone delivery. Apple's [APNs environment entitlement](https://developer.apple.com/documentation/bundleresources/entitlements/aps-environment), Expo's [native Application environment API](https://docs.expo.dev/versions/latest/sdk/application/), and [direct push delivery guide](https://docs.expo.dev/push-notifications/sending-notifications-custom/) document these native boundaries.
