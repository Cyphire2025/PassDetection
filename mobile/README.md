...
Build and install a new signed production APK containing this change. Existing Expo tokens cannot be converted to FCM tokens on the VPS. On the first online registration after upgrading, the existing login and installation are reused and the legacy Expo registration marker is replaced by the native FCM marker. The Metro adapter suppresses Expo's import-time relay auto-registration and clears its legacy preference when Android registration runs; it does not clear trip data or authentication. Token refresh, foreground retries, permission checks and sign-out de-registration continue to use the authenticated backend.

Phone push delivery is currently supported on Android only. iOS shows an explicit status directing users to in-app Updates and their travel team for urgent changes; its native APNs token is never submitted as an FCM token. In-app updates remain available. Validate foreground, background and closed-app taps and notification permission/channel settings on a physical Android device after installing the new APK; unit tests do not prove actual phone delivery.
