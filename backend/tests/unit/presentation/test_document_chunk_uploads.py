...
async def test_advisory_lock_is_stable_transaction_scoped_and_precedes_final_rereads() -> None:
    ...
    for handler in (analyze_and_rename_documents, upload_documents):
        ...
        assert (
            source.index("acquire_document_upload_advisory_lock")
...
async def test_distribution_scope_lock_is_stable_and_precedes_upload_lock() -> None:
    ...
    assert (
        source.index("acquire_document_upload_scope_advisory_lock")
        < source.index("acquire_document_upload_advisory_lock")
        < source.index("serialized_batch_result")
    )
...
def test_database_constraints_fence_concurrent_chunk_index_and_manifest_shape() -> None:
    ...
    assert (
        ...
        in constraints["ck_document_upload_chunks_index_manifest"]
    )
    assert (
        "expected_file_count <= expected_chunk_count * 50"
        in constraints["ck_document_upload_chunks_manifest_capacity"]
    )
    assert "file_count BETWEEN 1 AND 50" in constraints["ck_document_upload_chunks_file_count"]
    assert (
        "accepted_count + rejected_count = file_count"
        in constraints["ck_document_upload_chunks_result_counts"]
    )
    assert "flight_ticket_domestic" in constraints["ck_document_upload_chunks_scope"]
    assert "flight_ticket_domestic_arrival" in constraints["ck_document_upload_chunks_scope"]
