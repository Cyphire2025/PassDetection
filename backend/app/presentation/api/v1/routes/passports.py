...
# Compatibility facade: tests and sibling route modules import these names from
# `routes.passports`, while their cohesive implementations now live separately.
...
_apply_passport_excel_row_to_submission = (
    _excel_support._apply_passport_excel_row_to_submission
)
...
_select_whatsapp_tracking_export_payload = (
    _export_support._select_whatsapp_tracking_export_payload
)
...
async def _validated_upload_file(file: UploadFile, *, label: str) -> ValidatedUpload:
    ...
    try:
        while True:
            ...
            chunk = await file.read(
                min(PASSPORT_EXCEL_UPLOAD_READ_CHUNK_BYTES, remaining)
            )
            if not chunk:
                break
...
async def export_passports_by_group(
    ...
    if resolved_agency_match_field:
        ...
        agency_match_options = {
            str(field["key"]): field
            for field in agency_match_catalog
        }
        agency_match_option = agency_match_options.get(
            resolved_agency_match_field
        )
        ...
        requested_field_keys = [
            ...
            if key != resolved_agency_match_field
        ]
...
async def import_passports_by_group(
    ...
    try:
        existing_indexes = _build_passport_excel_existing_indexes(
            existing_submissions
        )
        resolved_rows: list[
            tuple[ImportedPassportRow, PassportSubmissionModel | None]
        ] = []
...
def _crop_response(
    *,
    ...
    sharpness_algorithm_version = (
        effective.sharpness_algorithm_version if effective else 1
    )
    ...
    return PassportImageCropResponse(
        ...
        sharpness_algorithm_version=cast(
            Literal[1, 2], sharpness_algorithm_version
        ),
...
async def get_passport_image_edit_source(
    ...
    return await private_object_streaming_response(
        ...
        content_disposition=(
            f'inline; filename="{image_type.value}-edit-source{extension}"'
        ),
        range_header=range_header,
    )
...
async def get_passport_image_original(
    ...
    return await private_object_streaming_response(
        ...
        media_type=content_type,
        content_disposition=(
            f'inline; filename="{image_type.value}-original{extension}"'
        ),
        range_header=range_header,
    )
