...
def _build_passport_excel_existing_indexes(
    ...
    return _PassportExcelExistingIndexes(
        ...
        by_identity={
            ...
            for identity, candidates in by_identity_lists.items()
        },
    )


def _passport_number_for_import_row(row: ImportedPassportRow) -> str | None:
    return _canonical_excel_passport_number(
        row.confirmed_fields.get("passport_number")
    )
...
def _resolve_existing_passport_excel_submission(
    ...
    staff_code = _staff_code_for_import_row(row)
    passport_match = (
        indexes.by_passport_number.get(passport_number) if passport_number else None
    )
    ...
    if len(identity_matches) > 1:
        raise _PassportExcelImportConflict(
            "A workbook row matches multiple existing passengers by legacy "
    ...
    if staff_code and existing_staff_code and staff_code != existing_staff_code:
        raise _PassportExcelImportConflict(
            "A workbook row conflicts with the staff code stored for its legacy "
            "name/contact match."
        )
