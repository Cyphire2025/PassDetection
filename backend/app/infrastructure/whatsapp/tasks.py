...
from app.infrastructure.whatsapp.welcome_recovery import run_stale_welcome_recovery
...
def process_document_whatsapp_broadcast(
    ...
    *,
    ...
    try:
        celery_async_runtime.run(run_document_whatsapp_broadcast(send_batch_id=send_batch_id))
    except Exception as exc:
        logger.error(
...
def process_qr_whatsapp_broadcast(
    ...
    *,
    ...
    try:
        celery_async_runtime.run(run_qr_whatsapp_broadcast(send_batch_id=send_batch_id))
    except Exception as exc:
        logger.error(
        ...
        if self.request.retries >= self.max_retries:
            celery_async_runtime.run(
                mark_qr_batch_failed(
                    send_batch_id=send_batch_id,
                    error_message=(
                        "WHATSAPP_WORKER_FAILED: QR delivery worker failed after bounded retries"
                    ),
                )
            )
            raise
...
@celery_app.task(  # type: ignore[untyped-decorator]
    bind=True,
    name="whatsapp.process_traveller_welcome_broadcast",
    queue="whatsapp",
    max_retries=2,
    default_retry_delay=10,
)
def process_traveller_welcome_broadcast(self: _BoundTask, *, batch_id: str) -> None:
    try:
        celery_async_runtime.run(run_traveller_welcome_broadcast(batch_id=batch_id))
    except Exception as exc:
        logger.error(
            "traveller_welcome_task_failed batch_id=%s error_type=%s", batch_id, type(exc).__name__
        )
        if self.request.retries >= self.max_retries:
            celery_async_runtime.run(
                mark_traveller_welcome_batch_failed(
                    batch_id=batch_id,
                    error_message="WhatsApp welcome worker failed after bounded retries.",
                )
            )
            raise
        raise self.retry(exc=exc) from exc
